...
{{- if $registry -}}
{{- printf "%s/%s:%s" ($registry | trimSuffix "/") $b.image.repository $b.image.tag -}}
{{- else -}}
...
of sync with each other. Defaults to Caddy's own routed host (appDomain) on
the standard HTTPS port; override frontend.origin (including a non-standard
port, e.g. for port-forward testing) when that's not how it's being reached. */}}
{{- define "frontend.origin" -}}
{{- .Values.frontend.origin | default (printf "https://app.%s" .Values.appDomain) -}}
{{- end }}
...
{{- .Values.frontend.oidc.authorizationEndpoint | default (printf "%s/auth" (include "dex.issuerUrl" .)) -}}
{{- end }}
