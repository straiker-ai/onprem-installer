...
{{/*
Image for the model-pull init container's/DaemonSet's cloud-storage sync — a
small official CLI image, picked by cloud provider (no combined aws+gsutil
image exists). Not mirrored through global.dockerRegistry since it's a
third-party image pulled directly from its own public registry, same as
upstream's own convention here.
*/}}
...
{{/* Applied to every container in this chart (main vllm process, both init
containers, and the DaemonSet's sync container). Deliberately does NOT set
