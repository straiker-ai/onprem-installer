...
# See install-straiker.sh — named EDGE_INFRA_* to avoid colliding with its
# unrelated charts/straiker-edge Helm-chart identifiers.
...
# edge-infra-destroy must run before eks-cluster-destroy: its Terraform
# config looks up the cluster's own VPC via `data "aws_eks_cluster"` (see
# terraform/aws/edge/network.tf) to resolve subnet IDs for the resources
# being destroyed, which fails once the cluster itself is gone.
declare -a ALL_PHASES=("preflight" "helm-uninstall" "namespace-cleanup" "edge-infra-destroy" "eks-cluster-destroy" "eks-tfbe-destroy")
...
Uninstaller phases:
  ...
  4) edge-infra-destroy  - (optional, requires --destroy-eks) destroy any temporary
                            self-signed certificate/bastion terraform/aws/edge created for
                            edgeType=alb (AWS only; no-op on GKE or if none were created).
  5) eks-cluster-destroy - (optional, requires --destroy-eks) destroy the EKS cluster via Terraform.
  6) eks-tfbe-destroy    - (optional, requires --destroy-eks) destroy the Terraform state bucket.

Notes:
  - Phases 4-6 only run with --destroy-eks. Phases 5 and 6 additionally require install
    state to confirm this installer provisioned EKS, and ask you to type the cluster name
    to confirm unless --yes is also passed. This permanently deletes cloud infrastructure.
EOF
}
...
# any state already there untouched. The .json copy is best-effort
...
# terraform/aws/edge — cleans up everything install-straiker.sh's edge-infra
# phase created for edgeType=alb: the AWS Load Balancer Controller's IAM
# role (unconditional) and the temporary self-signed cert/bastion (if
# chosen). Piggybacks on --destroy-eks rather than its own flag (already the
# "also tear down cloud infra beyond Helm" boolean; a third flag for this
# one small module isn't warranted). Simpler than phase_eks_cluster_destroy
# — no Karpenter node draining or post-destroy verification needed.
phase_edge_infra_destroy() {
  if [[ "${DESTROY_EKS}" != true ]]; then
    log "Skipping edge infra teardown; pass --destroy-eks to also destroy it. Nothing was touched."
    set_phase_status "${CURRENT_PHASE}" "skipped" "--destroy-eks not set."
    return
  fi
  resolve_destroy_inputs
  if [[ "${CLOUD_PROVIDER}" == "gke" ]]; then
    log "Edge infra (terraform/aws/edge) is AWS-only; nothing to destroy on GKE."
    set_phase_status "${CURRENT_PHASE}" "skipped" "cloud_provider=gke."
    return
  fi
  ...
  if [[ -z "${AWS_REGION}" || -z "${CLUSTER_NAME}" || -z "${TF_PREFIX}" ]]; then
    mark_phase_blocked "Missing region/cluster-name/tf-prefix (not in install state and not passed explicitly)."
    return
  fi
  ...
  tofu -chdir="${EDGE_INFRA_DIR}" destroy -auto-approve -input=false \
...
run_phase() {
  ...
  case "${phase}" in
    ...
    edge-infra-destroy) phase_edge_infra_destroy ;;
