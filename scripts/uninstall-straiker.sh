...
# release per model, named 'inference-<model>') and straiker-defend are
# always listed regardless of which products were actually selected at
# install time; uninstalling a release that was never installed is already a
# no-op (see phase_helm_uninstall).
declare -a ALL_RELEASES=("straiker-edge" "straiker-core" "inference-antman" "inference-hulk" "inference-quicksilver" "inference-thor" "inference-vision" "inference-wanda" "inference-thanos" "straiker-defend" "straiker-system" "straiker-artifact")
...
phase_eks_tfbe_destroy_gke() {
  ...
  if [[ -z "${GCP_REGION}" || -z "${GCP_PROJECT_OPT}" || -z "${TF_PREFIX}" ]]; then
    mark_phase_blocked "Missing gcp-region/gcp-project/tf-prefix (not in install state and not passed explicitly)."
    return
  fi

  if [[ ! -d "${GCP_TFBE_DIR}/.terraform" ]]; then
    local bucket_name
    bucket_name="$(get_install_metadata "bootstrap_bucket")"
    [[ -n "${bucket_name}" ]] || bucket_name="$(resolve_bucket_name_gcp)"
    mark_phase_blocked "No local Terraform state for the bootstrap bucket on this machine (it was never remote). If you want it gone, delete it manually: gcloud storage rm --recursive gs://${bucket_name}"
    return
  fi

  sync_terraform_workdir "${GCP_TFBE_SRC}" "${GCP_TFBE_DIR}"
  tofu -chdir="${GCP_TFBE_DIR}" init -upgrade -input=false
  tofu -chdir="${GCP_TFBE_DIR}" destroy \
