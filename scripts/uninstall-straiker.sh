...
DEFAULT_RELEASE="straiker-system"
...
declare -a ALL_PHASES=("preflight" "helm-uninstall" "secret-cleanup" "namespace-cleanup" "eks-cluster-destroy" "eks-tfbe-destroy")
...
Options:
  ...
  --release <name>               Helm release name (default: straiker-system). Straiker charts
                                  are installed as separate releases (straiker-system,
                                  straiker-core, straiker-artifact, ...) in the same namespace —
                                  re-run with --release for each one you want removed.
  --timeout <duration>           Helm wait timeout (default: 15m).
  --registry-secret-name <name>  Pull secret to delete when --delete-registry-secret is set.
  --delete-registry-secret       Delete registry pull secret from namespace.
  --delete-namespace             Delete namespace after uninstall.
...
Uninstaller phases:
  ...
  2) helm-uninstall      - Uninstall Helm release (wait for completion).
  3) secret-cleanup      - Optionally delete registry secret.
  4) namespace-cleanup   - Optionally delete namespace.
...
Notes:
  - Phases 5 and 6 only run with --destroy-eks, and only if install state confirms this
...
phase_helm_uninstall() {
  if helm status "${RELEASE_NAME}" -n "${NAMESPACE}" >/dev/null 2>&1; then
    ...
    log "Release '${RELEASE_NAME}' does not exist in namespace '${NAMESPACE}'; skipping."
  fi
}

phase_secret_cleanup() {
  if [[ "${DELETE_REGISTRY_SECRET}" != true ]]; then
    log "Skipping secret cleanup; pass --delete-registry-secret to enable."
    return
  fi
...
run_phase() {
  ...
  case "${phase}" in
    ...
    secret-cleanup) phase_secret_cleanup ;;
...
main() {
  ...
  update_state "${STATE_SECTION}" "" "in_progress" "namespace=${NAMESPACE}, release=${RELEASE_NAME}"
