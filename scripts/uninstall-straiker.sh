...
# terraform/aws/edge — cleans up the temporary self-signed cert/bastion
# install-straiker.sh's edge-infra phase may have created for edgeType=alb.
# Piggybacks on --destroy-eks rather than its own flag (already the "also
# tear down cloud infra beyond Helm" boolean; a third flag for this one
# small module isn't warranted). Simpler than phase_eks_cluster_destroy — no
# Karpenter node draining or post-destroy verification needed.
