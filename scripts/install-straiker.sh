...
declare -a ALL_PHASES=("eks-tfbe" "eks-cluster" "karpenter" "k8s-preflight" "aws-artifacts" "artifacts-sync" "straiker-system" "straiker-core" "straiker-inference")
...
Installer phases:
  ...
  8) straiker-core        - install/upgrade release 'straiker-core' (frontend, bifrost, glauth,
  ...
  9) straiker-inference   - install/upgrade one release per GPU model the selected products
                             ...
                             hulk/quicksilver/thor, Ascend's iris needs thanos). Each model is
                             its own release (release name = model name), so one model's
                             GPU-capacity problem doesn't block the others. Skipped entirely if
                             no selected product needs a model.
...
phase_straiker_system() {
  ...
  # NodePool/EC2NodeClass have no namespace of their own, so a prior install's
  # release-namespace annotation (e.g. before an INFRA_NAMESPACE rename) goes
  # stale independently of whatever k8s namespace holds the rest of the
  # release. Only covers charts/straiker-system's default straiker-{gpu,argus,
  # iris,shared} names — a customer overriding those via --values needs the
  # same manual fix for their custom names. No-ops for names that don't exist
  # (e.g. argus/iris NodePools not selected via --products).
  local nc_name
  for nc_name in straiker-gpu straiker-argus straiker-iris straiker-shared; do
    adopt_existing_resource "nodepool" "${nc_name}" ""
    adopt_existing_resource "ec2nodeclass" "${nc_name}" ""
  done
...
phase_straiker_inference() {
  ...
  for model in "${models[@]}"; do
    ...
    local cmd=(
      ...
      --set "global.modelBucket=s3://${models_bucket}"
      --wait
      --timeout "${HELM_TIMEOUT}"
    )
    if [[ -n "${CHART_VERSION}" ]]; then
...
# right after phase_straiker_core actually executes, since that phase gets
# skipped entirely on a re-run once already marked done. Reads live cluster
# state (Caddy's own ConfigMap) rather than assuming anything was just
# installed, so it prints correctly whether this run did real work or
# everything was already up from a previous one.
show_access_info() {
  local app_addr app_host app_port
  app_addr="$(kubectl -n "${INFRA_NAMESPACE}" get configmap caddy-config -o jsonpath='{.data.Caddyfile}' 2>/dev/null | grep -o 'app\.[^ {]*' | head -1 || true)"
  if [[ -n "${app_addr}" ]]; then
    app_host="${app_addr%%:*}"
    app_port="${app_addr##*:}"
    log ""
    ...
    log "  2. kubectl -n ${INFRA_NAMESPACE} port-forward svc/caddy-service ${app_port}:${app_port}"
