...
# GKE equivalent — no native add-on exists (see charts/straiker-system's
# karpenter-*-gke.yaml comments); this installs the community project's own
# Helm chart, using the Workload Identity binding terraform/gcp/gke/karpenter.tf
# already set up. A traditional Helm repo (not OCI), two charts (CRDs
# separate from the controller), and namespace "karpenter-system" — all per
# the project's own docs, not guessed:
...
# completion in phase_artifacts_sync, before straiker-system — that chart's
...
# release per model (matches the chart's own "one release per model" design),
# installed independently rather than bundled into one umbrella so each can
# apply/fail/be checked on its own. Which models run is derived from
# PRODUCTS_OPT each time (Defend -> antman/hulk/quicksilver/thor, Ascend ->
# thanos), not persisted separately, so it can never drift from the actual
# product selection. Fixed ServiceAccount name the chart's own Pod Identity
...
# straiker-inference (its LLM endpoints must already resolve) and
...
# Always runs (the frontend route is needed regardless of product
# selection); the defend-api route just 502s until straiker-defend is
# actually installed, same as any other not-yet-installed target.
EDGE_RELEASE="straiker-edge"
EDGE_CHART_NAME="straiker-edge"

# Which cloud this install targets — aws (EKS) or gke. Determines which
# terraform/<provider>/ modules and which cloud CLI (aws vs gcloud) every
# phase below actually calls.
#
# Unlike products/install_eks (defaults that can be reconsidered on a later
# run), this is a one-way foundational choice: everything from here down —
# which Terraform state, which cloud APIs, which IAM model — depends on it.
# So unlike every other flag in this script, --cloud-provider is a
# first-run-only prerequisite: capture_install_config below only ever reads
# it before "cloud_provider_set" exists in state; once persisted, the flag
# is simply not consulted again on later invocations (no comparison, no
# override, no error to design/maintain) — the state file always wins. A
# customer who genuinely needs to switch providers deletes
# ~/.straiker/install.json and starts over; there's no in-place migration
# path since the two providers' resources are entirely disjoint anyway.
...
# GCP equivalents of the AWS_REGION/AWS_PROFILE_OPT pair above — only
# consulted when CLOUD_PROVIDER=gke. gcloud has no per-command --profile
# flag the way aws-cli does; `gcloud auth login` / `gcloud config set
# project` are expected to already be set up in the caller's environment
# (GCP_PROJECT_OPT just tells terraform/phases which project to target, not
...
Options:
  ...
  --products <list>               Comma-separated products to provision infra for: ascend,
                                   ...
                                   one. Determines which Karpenter NodePools straiker-system
...
Installer phases:
  ...
  6) artifacts-sync       - install/upgrade release 'straiker-artifact' (its own chart), which
                             ...
                             time to check again — since straiker-system's workloads pull
...
phase_eks_cluster_gke() {
  ...
  local cluster_name cluster_endpoint karpenter_sa karpenter_node_sa
  ...
  gcloud container clusters get-credentials "${cluster_name}" --region "${GCP_REGION}" --project "${GCP_PROJECT_OPT}" >/dev/null
  ...
  if ! kubectl cluster-info >/dev/null 2>&1; then
    ...
    if [[ "${CLOUD_PROVIDER}" == "gke" ]]; then
      if command -v gcloud >/dev/null 2>&1 && ! gcloud auth print-access-token >/dev/null 2>&1; then
        ...
        mark_phase_blocked "Kubernetes API not reachable even after eks-cluster ran. Check: gcloud container clusters get-credentials ${CLUSTER_NAME} --region ${GCP_REGION} --project ${GCP_PROJECT_OPT}."
      else
        mark_phase_blocked "Kubernetes API not reachable. Point kubectl at your existing cluster (gcloud container clusters get-credentials <name> --region <region> --project <project>), or re-run with --install-eks to provision one."
      fi
    else
      if command -v aws >/dev/null 2>&1 && ! aws sts get-caller-identity >/dev/null 2>&1; then
