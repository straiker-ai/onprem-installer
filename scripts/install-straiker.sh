...
phase_straiker_core_helm() {
  ...
  # straiker-system's postgres/opensearch, so a straiker-system-helm that
  ...
  # a chance to update it) catches a straiker-system-helm that failed, was
  ...
  if [[ "${infra_status}" != "deployed" ]]; then
    mark_phase_blocked "Release '${INFRA_RELEASE}' in namespace '${INFRA_NAMESPACE}' is not healthy (status: '${infra_status:-not installed}'). straiker-core-helm needs its postgres/opensearch — fix and re-run 'straiker-system-helm' first (--phase straiker-system-helm --rerun-phase)."
    return
  fi
