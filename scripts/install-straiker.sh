...
phase_straiker_core() {
  ...
  if [[ "${EDGE_TYPE}" == "none" || "${EDGE_TYPE}" == "alb" ]]; then
    ...
    if [[ -n "${custom_domain}" ]]; then
      cmd+=(--set "straiker-frontend.frontend.origin=https://$(custom_origin_hostname straiker)")
      if [[ ",${PRODUCTS_OPT}," == *",ascend,"* ]]; then
        cmd+=(--set "straiker-frontend.frontend.irisControlPlanePublicEndpoint=https://$(custom_origin_hostname straiker-ascend)")
      fi
    fi
  fi
...
phase_straiker_ascend() {
  ...
  if [[ "${EDGE_TYPE}" == "none" || "${EDGE_TYPE}" == "alb" ]]; then
    ...
    custom_domain="$(get_metadata "custom_origin_domain")"
    if [[ -n "${custom_domain}" ]]; then
      cmd+=(--set "dataExfiltrationEndpoint=https://$(custom_origin_hostname straiker-ascend)")
      cmd+=(--set "corsAllowOrigins=https://$(custom_origin_hostname straiker)")
    fi
  fi
...
phase_edge_infra() {
  ...
  san_hostnames_json="$(python3 -c 'import json,sys; print(json.dumps(sys.argv[1:]))' \
    "$(custom_origin_hostname straiker)" "$(custom_origin_hostname straiker-defend)" "$(custom_origin_hostname straiker-ascend)")"
...
show_access_info() {
  if [[ "${EDGE_TYPE}" == "none" ]]; then
    ...
    if [[ -n "${custom_domain}" ]]; then
      ...
      log "  Frontend : https://$(custom_origin_hostname straiker)/"
      log "  Defend   : https://$(custom_origin_hostname straiker-defend)/"
      log "  Ascend   : https://$(custom_origin_hostname straiker-ascend)/"
      log ""
...
How should straiker-edge expose app/defend/ascend externally?
  ...
  5) alb                — one Ingress covering all routes via host-based
                          ...
                          You'll only be asked for your domain next. No real
                          ACM certificate needed up front — a temporary
                          self-signed one is generated automatically unless
                          you pass --alb-certificate-arn. The allowed CIDR
