...
Options:
  ...
  --ai-provider-mode <own-keys|bedrock|trial-key>
                                   ...
                                   Identity — the installer creates a dedicated IAM role with
                                   bedrock:InvokeModel and binds it to bifrost's ServiceAccount,
                                   no static keys required), or trial-key (a Straiker-hosted
                                   temporary virtual key, up to 30 days, PoC/trial use only,
                                   fetched automatically — no separate key to hold). Unlike
                                   --cloud-provider/--provision-strategy, safe to change on a
                                   later run — just pass this flag again and re-run
...
phase_straiker_ascend() {
  ...
  # IDs the codebase ever asks for today (see values.yaml's own comment) —
  # matches 1:1 with the "Bedrock > Model access" grants the operator was
  # already told to request earlier in this phase.
  if [[ "${AI_PROVIDER_MODE}" == "bedrock" ]]; then
    cmd+=(--set "availableModels[0]=bedrock/anthropic.claude-3-5-haiku-20241022-v1:0")
    cmd+=(--set "availableModels[1]=bedrock/anthropic.claude-3-5-sonnet-20241022-v2:0")
  fi
...
how it should reach one:
  ...
  2) bedrock   — use AWS Bedrock via EKS Pod Identity. The installer
                 ...
                 access to these two models in the AWS Console (Bedrock >
                 Model access) — Ascend won't use any others:
...
Make sure both are actually offered in the AWS region you're installing
into — Bedrock model availability varies by region.
