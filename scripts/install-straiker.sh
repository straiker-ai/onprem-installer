...
declare -a ALL_PHASES=("eks-tfbe" "eks-cluster" "karpenter" "k8s-preflight" "aws-artifacts" "artifacts-sync" "straiker-system" "shared-secrets" "ai-provider-secrets" "straiker-core" "straiker-inference" "straiker-defend" "straiker-ascend" "tailscale-operator" "edge-infra" "alb-controller" "straiker-edge")
...
Options:
  ...
  --edge-type <internal|none|tailscale|straiker-tailscale|alb>
                                   ...
                                   Balancer — for customers with no public internet access
                                   (e.g. accessed via WorkSpaces Secure Browser). The AWS
                                   Load Balancer Controller itself is installed automatically
                                   (phase 'alb-controller', IAM via terraform/aws/edge)
                                   unless an 'alb' IngressClass already exists. See
  ...
  --tailscale-account-mode <own|shared>
                                   ...
                                   selected.
  --custom-domain <domain>         Used when --edge-type none or --edge-type alb is
...
Installer phases:
 ...
 14) tailscale-operator   - only when --edge-type tailscale is selected. Installs the
                             ...
                             straiker-edge's Ingresses (mode 17 below) need.
 15) edge-infra           - only when --edge-type alb is selected. Runs terraform/aws/edge,
                             which unconditionally creates the AWS Load Balancer
                             Controller's IAM role (phase 16, alb-controller, needs this
                             regardless of the cert choice below), plus — when
                             --alb-self-signed-cert applies (the default unless
                             --alb-certificate-arn is given) — a temporary self-signed
                             certificate imported into ACM, and optionally
 ...
 16) alb-controller       - only when --edge-type alb is selected. Installs the AWS Load
                             Balancer Controller via its Helm chart, using the IAM role
                             ...
                             IngressClass straiker-edge's Ingress (mode 17 below) needs.
 17) straiker-edge        - install/upgrade release 'straiker-edge' (charts/straiker-edge).
                             ...
                             Load Balancer Controller installed in phase 16.
...
# only relevant for EDGE_TYPE=alb. terraform/aws/edge's alb_controller.tf
# (run by phase_edge_infra, which must run first) creates the IAM role/Pod
# Identity association this needs; this phase just points the chart at the
# matching ServiceAccount name (Pod Identity binds by namespace+name, no
# annotation needed, unlike IRSA). Skips entirely if an 'alb' IngressClass
# already exists — could be
...
phase_alb_controller() {
  if [[ "${EDGE_TYPE}" != "alb" ]]; then
    log "Edge type is '${EDGE_TYPE:-none}', not 'alb' — skipping the AWS Load Balancer Controller."
    set_phase_status "${CURRENT_PHASE}" "skipped" "EDGE_TYPE != alb."
    return
  fi
...
# terraform/aws/edge — runs for every EDGE_TYPE=alb install, not just the
# self-signed-cert path: it unconditionally creates the AWS Load Balancer
# Controller's IAM role (phase_alb_controller needs this, and every alb
# install needs the controller regardless of which cert was chosen), plus
# conditionally a temporary self-signed certificate + a tiny SSM-accessible
# verification bastion when requested (see capture_install_config's alb
# block). Runs before phase_alb_controller/phase_straiker_edge so
# alb_controller_role_arn/alb_certificate_arn are populated by the time
# those need them. Uses Terraform (not raw `aws` CLI calls) so
# uninstall-straiker.sh's `tofu destroy` can clean all of this up too — see
# this repo's plan notes for the full reasoning (safe-handoff
# `ignore_changes` rule on the ACM resource, so a later apply here can never
# revert a real certificate the customer has since imported into the same
# ARN).
phase_edge_infra() {
  require_command aws
  require_command tofu
  ...
  if [[ "${EDGE_TYPE}" != "alb" ]]; then
    log "Edge type is '${EDGE_TYPE:-none}', not 'alb' — skipping edge infra."
    set_phase_status "${CURRENT_PHASE}" "skipped" "EDGE_TYPE != alb."
    return
  fi
  ...
  local self_signed
  self_signed="$(get_metadata "alb_self_signed_cert")"
  ...
  tofu -chdir="${EDGE_INFRA_DIR}" apply -auto-approve -input=false \
    ...
    -var="prefix=${TF_PREFIX}" \
    -var="cluster_name=${CLUSTER_NAME}" \
    -var="enable_self_signed_cert=${self_signed}" \
  ...
  local controller_role_arn cert_arn bastion_id
...
EOF
    ...
    # Captured BEFORE falling back to metadata below — used later to tell
    # "an ARN explicitly passed/set this run" apart from "an ARN merely
    # already on record" (which, once phase_edge_infra has generated a
    # self-signed one, is itself an ARN — re-reading it here must not be
    # mistaken for the user opting out of self-signed on every later run).
    local explicit_alb_certificate_arn="${ALB_CERTIFICATE_ARN}"
    if [[ -z "${ALB_CERTIFICATE_ARN}" ]]; then
      ALB_CERTIFICATE_ARN="$(get_metadata "alb_certificate_arn")"
    fi
    if [[ -z "${ALB_SELF_SIGNED_CERT}" ]]; then
    ...
    # self-signed cert — e.g. a later run swapping in the real thing. Must
    # check the pre-fallback value: ALB_CERTIFICATE_ARN itself is non-empty
    # on every run once phase_edge_infra has generated a self-signed one,
    # which would otherwise permanently (and wrongly) flip this to false.
    if [[ -n "${explicit_alb_certificate_arn}" ]]; then
