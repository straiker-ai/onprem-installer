...
output "alb_controller_role_arn" {
  ...
  description = "ARN of the AWS Load Balancer Controller's IAM role. Always created (this module only runs for edgeType=alb or xalb, both of which need it)."
}
...
output "external_dns_role_arn" {
  value       = var.enable_route53_automation ? aws_iam_role.external_dns[0].arn : ""
  description = "ARN of ExternalDNS's IAM role. Empty when enable_route53_automation=false."
}
